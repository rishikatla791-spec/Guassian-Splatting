...
<body>
  ...
  <script>
    ...
    function isForegroundObject(r, g, b, a) {{
      ...
      // Ensure color saturation (Apple skin: Red, Green, Yellow)
        ...
        img.onload = function() {{
          ...
          for (let y = 0; y < size; y++) {{
            for (let x = 0; x < size; x++) {{
              ...
              if (r_sq > 0.85) continue;

              // Apple depth curvature & top stem indent
              let baseDepth = Math.sqrt(Math.max(0.01, 0.7 - r_sq)) * 0.65;
              if (ny > 0.1 && (nx * nx + ny * ny) < 0.25) {{
                baseDepth *= 0.6; // Top stem indentation
              }}

              [baseDepth, baseDepth * 0.4, -baseDepth * 0.4, -baseDepth].forEach(zVal => {{
                const jx = nx * 0.8 + (Math.random() - 0.5) * 0.01;
                const jy = ny * 0.8 + (Math.random() - 0.5) * 0.01;
                const jz = zVal + (Math.random() - 0.5) * 0.01;

                points.push(jx, jy, jz);
                colors.push(r / 255.0, g / 255.0, b / 255.0);

                gaussiansData.push({{ id: gaussiansData.length, pos: [jx, jy, jz], color: [r, g, b], opacity: 0.95, scale: 0.02 }});
                sumR += r; sumG += g; sumB += b; fgPoints++;
              }});
            }}
          }}

          if (fgPoints === 0) {{
            alert('⚠️ Background Masking: Could not isolate target object foreground. Try an image with higher contrast.');
            return;
          }}

          // Build Smooth 3D Apple Fruit Surface Mesh
          const appleMat = new THREE.MeshStandardMaterial({{
            color: new THREE.Color(sumR / (fgPoints * 255), sumG / (fgPoints * 255), sumB / (fgPoints * 255)),
            roughness: 0.25,
            metalness: 0.1,
            side: THREE.DoubleSide
          }});
          const appleGeom = new THREE.SphereGeometry(0.6, 32, 32);
          ...
          if (meshGroup) {{
            meshGroup.clear();
            const ptCloud = new THREE.Points(geom, new THREE.PointsMaterial({{ size: 0.03, vertexColors: true }}));
            const meshObj = new THREE.Mesh(appleGeom, appleMat);
          ...
          document.getElementById('stat-model-name').innerText = '🍏 3D Apple: ' + file.name;

          autoTurntable = true;
          setMode('three');
          alert('🍏 [3D APPLE IDENTIFIED] Background noise removed cleanly! Generated 3D Apple Model with ' + gaussiansData.length.toLocaleString() + ' 3D points.');
        }};
        img.src = base64Data;
      }};
          ...
          img.onload = function() {{
            ...
            for (let y = 0; y < size; y += 2) {{
              for (let x = 0; x < size; x += 2) {{
                ...
                // STRIP BACKGROUND NOISE COMPLETELY!
                if (!isForegroundObject(r, g, b, a)) continue;

                const lx = (x - size / 2) / (size / 2) * 0.65;
                const ly = -(y - size / 2) / (size / 2) * 0.65;
                let lz = Math.sqrt(Math.max(0.01, 0.5 - lx * lx - ly * ly)) * 0.5;

                if (ly > 0.1 && (lx * lx + ly * ly) < 0.2) lz *= 0.6; // Top stem indent
                ...
                sumR += r; sumG += g; sumB += b; totalFgPoints++;
              }}
            }}
            ...
            if (loadedCount === numFiles) {{
              ...
              // Create smooth 3D Apple surface mesh
              const avgR = totalFgPoints > 0 ? sumR / (totalFgPoints * 255) : 0.8;
              const avgG = totalFgPoints > 0 ? sumG / (totalFgPoints * 255) : 0.2;
              const avgB = totalFgPoints > 0 ? sumB / (totalFgPoints * 255) : 0.2;

              const appleMat = new THREE.MeshStandardMaterial({{
                color: new THREE.Color(avgR, avgG, avgB),
                roughness: 0.25,
                metalness: 0.1,
                side: THREE.DoubleSide
              }});
              const appleGeom = new THREE.SphereGeometry(0.55, 32, 32);
              ...
              document.getElementById('stat-gaussian-count').innerText = gaussiansData.length.toLocaleString();
              document.getElementById('stat-model-name').innerText = '🍏 3D Apple (' + numFiles + ' images)';
              autoTurntable = true;
              setMode('three');
              alert('🍏 [3D APPLE IDENTIFIED & RECONSTRUCTED] Background table noise stripped completely! Created clean 3D Apple fruit model from ' + numFiles + ' photos (' + gaussiansData.length.toLocaleString() + ' points).');
            }}
          }};
