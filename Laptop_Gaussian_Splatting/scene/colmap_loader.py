"""
COLMAP Binary and Text Format Parser.
Direct 1:1 match with official Inria 3DGS colmap_loader.py.
...
CameraStruct = collections.namedtuple("Camera", ["id", "model", "width", "height", "params"])
...
def read_next_bytes(fid, num_bytes, format_char_sequence, endian_character="<"):
    """Read and unpack binary bytes from file."""
    ...
    with open(path, "r") as fid:
        while True:
            ...
            if len(line) > 0 and line[0] != "#":
                ...
                cameras[camera_id] = CameraStruct(id=camera_id, model=model,
...
def read_cameras_binary(path_to_model_file: str) -> dict:
    ...
    with open(path_to_model_file, "rb") as fid:
        ...
        for _ in range(num_cameras):
            ...
            cameras[camera_id] = CameraStruct(id=camera_id,
                                              model=model_name,
                                              width=width,
                                              height=height,
...
def read_points3D_text(path: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Read COLMAP 3D points from text file."""
    xyzs, rgbs, errors = [], [], []
    with open(path, "r") as fid:
        for line in fid:
            line = line.strip()
...
def read_points3D_binary(path_to_model_file: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    with open(path_to_model_file, "rb") as fid:
        ...
        for p_id in range(num_points):
            ...
            fid.seek(track_length * 8, os.SEEK_CUR)  # Skip 2D track points
            xyzs[p_id] = xyz
            rgbs[p_id] = rgb
            errors[p_id] = error
...
# Official Inria function name aliases
