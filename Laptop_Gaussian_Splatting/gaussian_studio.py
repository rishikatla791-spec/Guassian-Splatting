...
# Add gaussian-splatting submodule to front of python path
GS_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "gaussian-splatting")
if GS_PATH in sys.path:
    sys.path.remove(GS_PATH)
...
def launch_viewer(model_dir):
    ...
    viewer_bin = str(script_dir / "viewers" / "bin" / "SIBR_gaussianViewer_app.exe")
    ...
    env = os.environ.copy()
    viewers_bin_dir = str(script_dir / "viewers" / "bin")
    env["PATH"] = f"{viewers_bin_dir};C:\\Users\\Rishi\\anaconda3\\envs\\gaussian_cuda\\bin;" + env.get("PATH", "")
...
def main():
    script_dir = Path(__file__).resolve().parent
    default_model = str(script_dir / "output" / "truck")
    default_out = str(script_dir / "output" / "truck_orbit360")
