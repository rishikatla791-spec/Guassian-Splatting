...
extern "C" void native_progress_callback(ProgressMessage msg, void* user_data) {
    if (g_cancel_requested.load(std::memory_order_relaxed)) {
        LOGI("On-Device 3DGS: Cancellation detected during training.");
        return;
    }
    ...
    if (msg.tag == 1) {
        if (msg.iter % 20 == 0 || msg.iter == g_total_steps) {
            ...
            if (g_jvm && g_listener_global && g_listener_mid) {
                JNIEnv* env = nullptr;
                jint get_res = g_jvm->GetEnv((void**)&env, JNI_VERSION_1_6);
        ...
        LOGI("On-Device 3DGS Training loop completed successfully.");
    }
}
...
// High-performance chunked binary PLY to 32-byte .splat converter
bool convert_ply_to_splat(const std::string& ply_path, const std::string& splat_path) {
    ...
    // Chunked batch reading: 4096 vertices per I/O burst
    const size_t BATCH_SIZE = 4096;
    std::vector<char> raw_batch(stride * BATCH_SIZE);
    std::vector<SplatEntry> splat_batch;
    splat_batch.reserve(BATCH_SIZE);

    const float C0 = 0.28209479177387814f;
    int total_written = 0;
    int vertices_remaining = num_vertices;

    while (vertices_remaining > 0) {
        size_t current_batch = std::min((size_t)vertices_remaining, BATCH_SIZE);
        file.read(raw_batch.data(), stride * current_batch);
        size_t bytes_read = file.gcount();
        size_t actual_vertices = bytes_read / stride;
        if (actual_vertices == 0) break;

        splat_batch.clear();
        for (size_t i = 0; i < actual_vertices; ++i) {
            const char* v_data = raw_batch.data() + i * stride;

            float x = (offset_x >= 0) ? *(float*)(v_data + offset_x) : 0.0f;
            float y = (offset_y >= 0) ? *(float*)(v_data + offset_y) : 0.0f;
            float z = (offset_z >= 0) ? *(float*)(v_data + offset_z) : 0.0f;

            // Numerical safety check
            if (!std::isfinite(x) || !std::isfinite(y) || !std::isfinite(z)) continue;

            float s0 = (offset_s0 >= 0) ? std::exp(*(float*)(v_data + offset_s0)) : 0.01f;
            float s1 = (offset_s1 >= 0) ? std::exp(*(float*)(v_data + offset_s1)) : 0.01f;
            float s2 = (offset_s2 >= 0) ? std::exp(*(float*)(v_data + offset_s2)) : 0.01f;

            float f_dc_0 = (offset_f0 >= 0) ? *(float*)(v_data + offset_f0) : 0.0f;
            float f_dc_1 = (offset_f1 >= 0) ? *(float*)(v_data + offset_f1) : 0.0f;
            float f_dc_2 = (offset_f2 >= 0) ? *(float*)(v_data + offset_f2) : 0.0f;
            ...
            float raw_op = (offset_op >= 0) ? *(float*)(v_data + offset_op) : 0.0f;
            float op = (1.0f / (1.0f + std::exp(-raw_op))) * 255.0f;
            uint8_t a = (uint8_t)std::max(0.0f, std::min(255.0f, op));

            float r0 = (offset_r0 >= 0) ? *(float*)(v_data + offset_r0) : 1.0f;
            float r1 = (offset_r1 >= 0) ? *(float*)(v_data + offset_r1) : 0.0f;
            float r2 = (offset_r2 >= 0) ? *(float*)(v_data + offset_r2) : 0.0f;
            float r3 = (offset_r3 >= 0) ? *(float*)(v_data + offset_r3) : 0.0f;
            ...
            splat_batch.push_back({ x, y, z, s0, s1, s2, r, g, b, a, q0, q1, q2, q3 });
        }

        if (!splat_batch.empty()) {
            out.write((const char*)splat_batch.data(), splat_batch.size() * sizeof(SplatEntry));
            total_written += splat_batch.size();
        }

        vertices_remaining -= actual_vertices;
    }

    LOGI("Chunked conversion complete: wrote %d Gaussians into %s (%d bytes)", total_written, splat_path.c_str(), total_written * 32);
    return true;
}
...
Java_com_splat_mobile3dgs_engine_NativeBrushEngine_nativeCheckVulkanEngine(
    ...
    std::string status = (fn != nullptr) 
        ? "READY: Vulkan compute engine symbol train_and_save resolved successfully."
        : "ERROR: Failed to resolve train_and_save from libbrush_c.so.";
    return env->NewStringUTF(status.c_str());
}

JNIEXPORT jboolean JNICALL
Java_com_splat_mobile3dgs_engine_NativeBrushEngine_nativeCancelTraining(
    JNIEnv *env, jobject thiz
) {
    g_cancel_requested.store(true, std::memory_order_relaxed);
    LOGI("NativeBrushEngine: Cancellation requested by caller.");
    return JNI_TRUE;
}
...
Java_com_splat_mobile3dgs_engine_NativeBrushEngine_nativeTrainAndSave(
    ...
    if (result == TrainSuccess && !g_cancel_requested.load(std::memory_order_relaxed)) {
