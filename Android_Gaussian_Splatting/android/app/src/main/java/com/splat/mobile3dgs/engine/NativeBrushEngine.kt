...
class NativeBrushEngine {
    ...
    fun cancelTraining(): Boolean {
        return if (isLoaded) {
            nativeCancelTraining()
        } else false
    }

    fun checkEngineDiagnostics(): String {
        return if (isLoaded) {
            nativeCheckVulkanEngine()
        } else {
            "NOT_LOADED: $loadError"
        }
    }
    ...
    private external fun nativeCancelTraining(): Boolean
